...
void render_body(SDL_Renderer *r, const Body *b) {
    ...
    if (b->shape.type == SHAPE_CIRCLE) {
        ...
        int width = (int)b->shape.rect.width;
        int height = (int)b->shape.rect.height;
        
        // Filled rectangle with body color
        render_rect_filled(r, (int)cx, (int)cy, width, height, b->color);
        
        // White outline for visibility
        render_rect(r, (int)cx, (int)cy, width, height, outline);
    }
}
